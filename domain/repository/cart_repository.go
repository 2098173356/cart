...
func (c CartRepository) DeleteCart(i int64) error {
...
func (c CartRepository) FindAll(i int64) (cartAll []model.Cart, err error) {
	return cartAll, c.mysqlDB.Where("user_id = ?", i).Find(&cartAll).Error
}
...
func (c CartRepository) DecrNum(i int64, i2 int64) error {
	cart := &model.Cart{ID: i}
	db := c.mysqlDB.Model(cart).Where("num>=?", i2).UpdateColumn("i2", gorm.Expr("i2-?", i2))
	if db.Error != nil {
		return db.Error
	}
	if db.RowsAffected == 0 {
